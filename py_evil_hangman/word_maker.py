...
class WordMakerAI():
    ...
    def __init__(self, words_file: str, verbose=False):
        ...
        with open(words_file) as words:
            for line in words:
                word= line.strip()   # cleans characters from the beginning and end of one string, removes \n
                #print(type(word))
                if len(word) > 0:    # checks for empty lines
                    word_length = len (word)

                    if word_length not in self.dict:   # if key not present in dict, create a key and list of empty values
    ...
    def guess(self, guess_letter) -> list[int]:
        ...
        for word in self.words_list:
            result=self.get_letter_positions_in_word(word, guess_letter)
        ...
        for key,value in self.families.items():
            if len(value) == largest_family:                            # look for all possible large values
                eligible_family.append((key,value))                    # append in list(tuple) with key, value
                                                                      ...
                                                                      # guess letter 'e'
                                                        #example: [((), ['book', 'fish']), ((2, 3), ['tree', 'bree'])]

        if len(eligible_family) > 1:     # if there is tie in large families
            eligible_family.sort(key=lambda x:len(x[0]))   # sort with length of tuple (), (1,) (1,1)..
        final_families= eligible_family[0]                              # choose the first one as its smallest

        self.words_list = list(final_families[1])   # updating words list after each guess

        return list(final_families[0])             # return (list) positions of where a guess_letter should appear
